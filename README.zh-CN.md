...
项目面向希望优先使用 Claude Code、并为 Codex 等未来工具保留扩展空间的个人和小团队。本地通过 `fclaude` 等命令经由 WireGuard 连接远端节点，通过 Mutagen 保持项目文件同步，并附加到受控 Docker 运行时中由 tmux 保持长期在线的 Agent shell。

## 仓库

- `agent-remote`：项目级部署包、架构方案和跨仓库文档。
- `agent-remote-protocol`：OpenAPI、JSON Schema、节点任务契约和共享术语。
...
Release workflow 会发布部署归档、CLI/Node 二进制、协议包、GHCR 镜像和 GitHub Release notes。
