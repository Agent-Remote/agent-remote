...
| --- | --- |
| Remote wrapper / local Bridge / Device Client | `0.1.8` |
...
`0.2.14`, which contains the schema-9 Bridge admission fix. Keep the root
...
The component record is now the published Bridge `0.1.8` promotion with
`production_ready=true` and no blockers. The stable root `0.2.25` release
...
EGO_BROWSER_EXPECTED_WRAPPER_VERSION=0.1.8
