...
that predate this change may continue to validate schemas 1-7 during migration; those legacy schemas
retain their original 30-day expiry and are never generated by current workflows.
...
The normal root `release` workflow invokes community evidence automatically. To regenerate the
signed manifest for the same immutable root tag without rebuilding source artifacts, dispatch the
same reusable workflow. This creates a new release-bound manifest; it does not extend or revive an
older schema 1-7 manifest:
